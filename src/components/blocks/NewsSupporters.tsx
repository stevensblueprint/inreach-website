                ...
                if (company?.logo && company?.name) {
                  return (
                    <Box key={i + company.name} p={15}>
                      <Image src={company.logo} alt={company.name + " Image"} className={cn("bg-white bg-clip-content shadow-lg", {"rounded-full max-w-[150px] hover:animate-bounceImage" : data.format === "circle", "rounded-lg max-w-[150px]" : data.format === "square"})}/>
                    </Box>
                  )
                }
              })

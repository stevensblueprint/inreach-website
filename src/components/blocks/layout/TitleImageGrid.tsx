		...
		<section className='w-full flex flex-col items-center py-6' style={{ background: bgColor }}>
			...
			{data.titleImageGridImage?.src && data.titleImageGridImage?.alt && (
				<div className='w-full max-h-[600px]'>
					<Image
						src={data.titleImageGridImage.src}
						alt={data.titleImageGridImage.alt}
						width={800}
						height={600}
						className='mx-auto'
					/>
			...
			{data.titleImageGridContent && data.titleImageGridContent?.length > 0 && (
				<SimpleGrid
					cols={{ md: data.titleGridNum ? data.titleGridNum : 1, base: 1, sm: 3 }}
					px={{ sm: 24, base: 16 }}
					className='max-w-7xl'
				>
					{data.titleImageGridContent.map((content, i) => {
						if (content)
							return (
								<div
									className={cn(
										'w-full prose prose-headings:m-0 prose-p:m-0 flex flex-col gap-2 text-center',
										content.contentTextColor
											? inputProseClasses[content.contentTextColor]
